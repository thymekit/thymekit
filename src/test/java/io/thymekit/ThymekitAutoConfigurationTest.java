...
class ThymekitAutoConfigurationTest {
    ...
    void theOneSwitchIsDescribedWhereItIsTyped() throws Exception {
        try (var metadata = getClass().getResourceAsStream("/META-INF/spring-configuration-metadata.json")) {
            assertThat(metadata).as("the metadata an ide reads, published in the jar").isNotNull();
            String described = new String(metadata.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
            assertThat(described).contains("\"thymekit.tidy.enabled\"")
                .contains("\"defaultValue\": true")
                .contains("formatting whitespace");
        }
    }
